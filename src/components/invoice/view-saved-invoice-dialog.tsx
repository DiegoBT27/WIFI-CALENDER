...
interface ViewSavedInvoiceDialogProps {
...
export function ViewSavedInvoiceDialog({ invoice, isOpen, onClose, onDelete }: ViewSavedInvoiceDialogProps) {
  ...
  const handlePrint = () => {
    ...
    if (contentToPrint) {
      ...
      if (printWindow) {
        ...
        // Re-use styles from InvoiceDialog or define specific ones here
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] grid grid-rows-[auto_minmax(0,1fr)_auto] gap-4">
        <DialogHeader>
          <DialogTitle>Factura Guardada: {invoice.id}</DialogTitle>
          <DialogDescription>
            Detalles de la factura para {invoice.customerName}. Fecha de Emisión: {invoice.invoiceDate}
          </DialogDescription>
        </DialogHeader>
          ...
          <div ref={invoiceContentRef} className="p-4 sm:p-6 bg-card text-card-foreground rounded-lg">
            <div className="flex justify-between items-start mb-8">
              ...
              <div className="text-right">
                ...
                <p className="text-sm text-muted-foreground">Nº: {invoice.id}</p>
                <p className="text-sm text-muted-foreground">Fecha: {invoice.invoiceDate}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div>
                ...
                {/* Phone number is not stored in SavedInvoice currently, add if needed */}
              </div>
            </div>
